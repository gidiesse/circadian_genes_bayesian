...
int main()
{
    std::string file_path = "C:/Users/aless/Downloads/";
    ...
    for (size_t i=0; i < 1; ++i) {
        ...
        arma::mat t_chol = chol_cov(veta_1);
        veta_1.brief_print();
        arma::mat Q,R;
        arma::qr(Q,R,t_chol);
        arma::mat S=arma::inv(R);
        arma::mat veta=S*S.t();
        arma::mat Meta=(((Y-B*veta).t())*lmsg)*veta;
        arma::mat eta=Meta+arma::randn(T,k)*S.t();
        // Update of Lambda (rue & held)
        for (size_t h=0; h < p; ++h) {
                      // This is the cov matrix as in the paper; REMOVED: Z*eye(Ttilde)*Z'+
...
arma::mat chol_cov (const arma::mat& mat) {
    ...
    if (mat.is_sympd()) {
        ...
        arma::mat T = eigvecs * Lambda * eigvecs.t();
    }
    return T;
}
